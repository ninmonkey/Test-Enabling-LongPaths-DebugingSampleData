...
( $scarySquaredPathFolder = Join-Path 'G:\temp\longpathTest\' -ChildPath 'stuff' @(
    ('a'..'g').ForEach({ [string]$_ * 240 -join ''}) ))

( Test-Path $scaryPathFolder ) ? '' : ( mkdir $scaryPathFolder )
( Test-Path $scarySquaredPathFolder ) ? '' : ( mkdir $scarySquaredPathFolder )
( $scaryLog = Join-String -f '{0}\SomeFile.log' -in $scaryPathFolder )#
( $scarySquaredLog = Join-String -f '{0}\SomeFile.log' -in $scarySquaredPathFolder )
Get-Date | Set-Content -path $scaryLog
Get-Date | Set-Content -path $scarySquaredLog
...
$info.ScarySquared            = $scarySquaredPathFolder
$info.ScarySquared_Length     = $scarySquaredPathFolder.length
$info.ScarySquared_SegLengths = $scarySquaredPathFolder -split '\\'
    | Join-String -p { $_.Length } -sep ', '
$info.ScaryLog            = $scaryLog
$info.ScaryLog_NameLength = (gi $scaryLog).FullName.Length
$Info.ScaryLog_FileSize   = ( gi $ScaryLog ).Length
$Info.ScarySquared_OldSyntax = 'G:\temp\LONGPA~1\stuff\AAAAAA~1\BBBBBB~1\CCCCCC~1\DDDDDD~1\EEEEEE~1\FFFFFF~1'
$ScarySquared_ResolvedFromOldSyntax = (gi 'G:\temp\LONGPA~1\stuff\AAAAAA~1\BBBBBB~1\CCCCCC~1\DDDDDD~1\EEEEEE~1\FFFFFF~1')
$Info.ScarySquared_ResolvedOld = (gi 'G:\temp\LONGPA~1\stuff\AAAAAA~1\BBBBBB~1\CCCCCC~1\DDDDDD~1\EEEEEE~1\FFFFFF~1').FullName
$Info.ScarySquared_ResolvedFromOld_Length = $ScarySquared_ResolvedFromOldSyntax.FullName.Length
